...
import Foundation
final class PostAPIManager: PostUseCase {
    
    private init() {}
    static let instance = PostAPIManager()
    
    func getPostList(completion: @escaping ((PostList?, CoreErrorModel?) -> Void)) {
        CoreAPIManager.instance.request(type: PostList.self, route: PostHelper.getPosts, completion: completion)
    }
    
    func createPost(
        body: [String : Any],
        completion: @escaping ((PostDTO?, CoreErrorModel?) -> Void)
    ) {
        CoreAPIManager.instance.request(type:PostDTO.self, route: PostHelper.createPosts(body), completion: completion)
    }
